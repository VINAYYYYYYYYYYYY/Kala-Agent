"""Optional FreeCAD smoke for the per-part runner (CTO Task 6).

Marked ``@pytest.mark.freecad`` and skipped when the FreeCAD Python module
is missing so default mock CI is not blocked. Does not edit
``kala/agent/loop.py``.
"""
...
@pytest.mark.freecad
def test_gearbox_part_runner_freecad_smoke(tmp_path, monkeypatch):
    """Gearbox brief on FreeCAD: done, last_export, ≥2 bodies when both bind."""
    ...
    agent = Agent(backend_name="freecad", planner=StubPlanner(), max_turns=32)
    result = agent.run(_GEARBOX_BRIEF)
    st = result.state

    if st.status == "error" and not st.history:
